...
def run(subject_id, session_id, sim_vertex, err_level, win_size, json_file):
    ...
    out_path = os.path.join(parameters['output_path'], 'coreg_err_bem_simulations')
    ...
    if not os.path.exists(output_file):
        ...
        ses_path = op.join(sub_path, 'bem_sim')
        ...
        tmp_dir = op.join(out_path, f'sim_coregerr_bem_{sim_vertex}_{err_level}_{win_size}')
        ...
        data_file = os.path.join(ses_path,
            f'bem_inverted_{subject_id}-{session_id}-motor-epo.mat'
        )
        ...
        shutil.copy(
            os.path.join(data_path, f'SPMgainmatrix_{data_base}_1.mat'),
            os.path.join(tmp_dir, f'SPMgainmatrix_{data_base}_1.mat')
        )
        ...
        with spm_context() as spm:
            ...
            for l in range(n_layers):
                ...
                for ext in extensions:
                    os.remove(
                        os.path.join(
                            tmp_dir, f'{prefix}bem_inverted_{subject_id}-{session_id}-motor-epo.{ext}'
                        )
                    )
                    os.remove(
                        os.path.join(
                            tmp_dir, f'm{prefix}bem_inverted_{subject_id}-{session_id}-motor-epo.{ext}'
                        )
                    )
