...
def run(subject_id, session_id, sim_vertex, snr, win_size, patch_size, json_file):

    with open(json_file) as pipeline_file:
        parameters = json.load(pipeline_file)

    out_path = os.path.join(parameters['output_path'], 'snr_patch_size_simulations')
    if not os.path.exists(out_path):
        os.mkdir(out_path)
    output_file = os.path.join(
        out_path,
        f"vx_{sim_vertex}_snr_{snr}_winsize_{win_size}_patch_size_{patch_size}.pickle"
    )
    ...
    if not os.path.exists(output_file):
        ...
        tmp_dir = op.join(out_path, f'sim_snr_{sim_vertex}_{snr}_{win_size}_{patch_size}')
        ...
        shutil.copy(os.path.join(surf_dir, f'FWHM5.00_multilayer.11.ds.link_vector.fixed.mat'),
                    os.path.join(tmp_dir, f'FWHM5.00_multilayer.11.ds.link_vector.fixed.mat'))
        shutil.copy(os.path.join(surf_dir, f'FWHM{patch_size:.2f}_multilayer.11.ds.link_vector.fixed.mat'),
                    os.path.join(tmp_dir, f'FWHM{patch_size:.2f}_multilayer.11.ds.link_vector.fixed.mat'))
        ...
        for layer in layers:
            ...
            shutil.copy(os.path.join(surf_dir, f'FWHM{patch_size:.2f}_{name}.mat'),
                        os.path.join(tmp_dir, f'FWHM{patch_size:.2f}_{name}.mat'))
        ...
        sim_vx_res['patch_size'] = patch_size
...
if __name__=='__main__':
    ...
    patch_size = all_patch_sizes[sim_idx]

    run(subject_id, session_id, vertex_idx, snr, win_size, patch_size, json_file)
