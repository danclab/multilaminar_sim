...
def run(subject_id, session_id, sim_vertex, err_level, win_size, json_file):
    ...
    out_path = os.path.join(parameters['output_path'], 'coreg_err_agm_simulations')
    ...
    if not os.path.exists(output_file):
        ...
        tmp_dir = op.join(out_path, f'sim_coregerr_agm_{sim_vertex}_{err_level}_{win_size}')
        ...
        agm_surf_dir = os.path.join(sub_path, 'agm_surf')
        os.makedirs(os.path.join(tmp_dir, 'agm_surf'), exist_ok=True)
        shutil.copy(os.path.join(agm_surf_dir, 'multilayer.11.ds.link_vector.fixed.gii'),
                    os.path.join(tmp_dir, 'agm_surf', 'multilayer.11.ds.link_vector.fixed.gii'))
        agm_multilayer_mesh_fname = os.path.join(tmp_dir, 'agm_surf', 'multilayer.11.ds.link_vector.fixed.gii')
        shutil.copy(os.path.join(agm_surf_dir, 'FWHM5.00_multilayer.11.ds.link_vector.fixed.mat'),
                    os.path.join(tmp_dir, 'agm_surf', 'FWHM5.00_multilayer.11.ds.link_vector.fixed.mat'))
        agm_layer_fnames = []
        for layer in layers:
            if layer == 1:
                name = f'pial.ds.{orientation_method}'
            ...
            shutil.copy(os.path.join(agm_surf_dir, f'{name}.gii'),
                        os.path.join(tmp_dir, 'agm_surf', f'{name}.gii'))
            agm_layer_fnames.append(os.path.join(tmp_dir, 'agm_surf', f'{name}.gii'))
            shutil.copy(os.path.join(agm_surf_dir, f'FWHM5.00_{name}.mat'),
                        os.path.join(tmp_dir, 'agm_surf', f'FWHM5.00_{name}.mat'))
        ...
        with spm_context() as spm:
            coregister(
                ...
                agm_multilayer_mesh_fname,
                base_fname, spm_instance=spm,
                viz=False
            )

            [_, _] = invert_ebb(
                agm_multilayer_mesh_fname, base_fname,
            ...
            for l in range(n_layers):
                ...
                layer_ts, ts_time, _ = load_source_time_series(
                    sim_fname,
                    mu_matrix=MU
                )
                prior_vertex = np.argmax(np.max(layer_ts, axis=-1), axis=0)
